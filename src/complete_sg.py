...
def main():
    ...
    parser.add_argument(
        "--visualize_partial",
        action="store_true",
        help="Visualize the partial scenes with masking"
    )
    parser.add_argument(
        "--eight_views",
        action="store_true",
    ...
    print("Complete scene graphs with the generative model")
    for epoch in range(args.n_epochs):
        for batch_idx, batch in tqdm(
            ...
            texts = []
            batch_selected_relations, batch_selected_descs, batch_selected_obj_ids = [], [], []
            for desc_idx, desc in enumerate(descriptions):
                ...
                fill_templates(desc,
                    ...
                    return_obj_ids=True  # to mask scenes for completion
                )
            ...
            with torch.no_grad():
                objs, edges, objfeat_vq_indices = model.complete(
                    batch, batch_selected_obj_ids,
            ...
            with torch.no_grad():
                boxes_pred = sg2sc_model.complete(
                    batch, batch_selected_obj_ids,
            ...
            for i in range(len(bbox_params_t)):
                ...
                trimesh_meshes_partial = [
                    trimesh_meshes[mesh_idx] for mesh_idx in range(len(trimesh_meshes)-1)  # the last one is the floor
                    if mesh_idx not in batch_selected_obj_ids[i] and mesh_idx < batch["lengths"][i].item()
                ]
                if bbox_meshes is not None and len(bbox_meshes) > 0:
                    ...
                    bbox_meshes_partial = bbox_meshes
                ...
                all_vertices_partial = np.concatenate([
                    tr_mesh.vertices for tr_mesh in trimesh_meshes_partial
                ], axis=0)
                x_max_partial, x_min_partial = all_vertices_partial[:, 0].max(), all_vertices_partial[:, 0].min()
                z_max_partial, z_min_partial = all_vertices_partial[:, 2].max(), all_vertices_partial[:, 2].min()

                tr_floor_partial, _ = floor_plan_from_scene(
                    raw_dataset[0], config["data"]["path_to_floor_plan_textures"],  # `raw_dataset[0]` is not really used
                    without_room_mask=True,
                    rectangle_floor=True, room_size=[x_min_partial, z_min_partial, x_max_partial, z_max_partial]
                )
                trimesh_meshes.append(tr_floor)
                trimesh_meshes_partial.append(tr_floor_partial)
                ...
                if args.visualize_partial:
                    os.system(f"rm -rf {tmp_dir_partial}")
                    if args.verbose:
                        print(f"Save the scene to {export_dir}\n")

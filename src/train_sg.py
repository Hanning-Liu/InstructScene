...
def main():
    parser = argparse.ArgumentParser(
        description="Train a generative model on scene graphs"
    )
    ...
    for i in range(start_epoch, epochs):
        ...
        for b, batch in zip(range(steps_per_epoch), yield_forever(train_loader)):
            ...
            optimizer.zero_grad()
            # Compute the loss
            losses = model.compute_losses(batch, text_last_hidden_state, text_embeds)
        ...
        if (i+1) % val_freq == 0:
            ...
            with torch.no_grad():
                for val_b, val_batch in enumerate(val_loader):
                    ...
                    # Prepare CLIP text embeddings
                    val_descriptions = val_batch["descriptions"]
                    val_texts = []
                    for desc_idx, val_desc in enumerate(val_descriptions):
                        val_text, _, _ = fill_templates(val_desc,
                            train_dataset.object_types,
                            train_dataset.predicate_types,
                            val_batch["object_descs"][desc_idx],
                            seed=None
                        )
                        val_texts.append(val_text)  # a batch of texts
                    val_text_last_hidden_state, val_text_embeddings = text_encoder(val_texts)
                    # Compute the loss
                    val_losses = model.compute_losses(val_batch, val_text_last_hidden_state, val_text_embeddings)
